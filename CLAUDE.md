...
# PROJECT: MollyArt portfolio

Everything above is the general standard. This section is the specific brief for the current
project and, per §2, the paintings and the Stitch design are the primary references.

## What this site is

An art portfolio for **MollyArt** — paintings by Mirjana. Personal archive/showcase, not a
shop. The paintings are the star; the site stays quiet, warm, and gallery-like.

## Palette (derived from the paintings)

- Background: warm cream / parchment `#F6EFE3`. Optionally a very subtle vintage ornament
  texture at low opacity — never louder than the paintings.
- Primary accent: deep burgundy `#6B1F2E` for headings and links.
- Fine details: muted gold `#C9A227` / warm brown `#8A6D3B`, used sparingly (borders,
  hover states).
- Nothing outside this family. The paintings supply all the color energy.

## Typography for this project

Elegant serif display for "MollyArt" and headings (Cormorant Garamond or Playfair Display),
plus one clean readable body face. Two typefaces maximum.

## Layout (fixed scope — do not add sections)

1. **Header** — "MollyArt" wordmark, small tagline with the artist's name, minimal nav
   (Paintings · About).
2. **Gallery** — masonry grid of paintings, mixed portrait/landscape orientations flowing
   naturally. Click/tap opens a **lightbox**: large image on dimmed background, close button,
   prev/next, keyboard arrows + Escape on desktop, swipe on mobile.
3. **About** — a few lines of bio with one small photo. No large hero photo of the artist.
4. **Footer** — link to the separate developer portfolio ("Also a developer →"),
   contact/social links.

Out of scope unless explicitly requested: shop/e-commerce, blog, newsletter, testimonials,
contact forms, anything else.

## Images

- Preserve aspect ratio always; never stretch or crop paintings oddly.
- Lazy-load; serve sensible sizes in the grid (no multi-MB originals), full quality in the
  lightbox.
- Alt text on every painting (title of the work).
- Mobile gallery: 1–2 columns; desktop: 3 columns max.

## Stack and code standards

- React + TypeScript. Typed props, no unnecessary `any`.
- Clear component names (Gallery, PaintingCard, Lightbox, About, Footer), sensible folder
  structure.
- Every image/network failure handled — no silent failures.
- Small, clean codebase. Resist adding a library where a few lines of CSS will do.
